...
prompt Installing Page 200 (Teacher Dashboard) into Application 105

DECLARE
  l_app_id NUMBER := 105; -- target application
  l_ui_id  NUMBER;
BEGIN
  -- Target existing APEX app 105
  apex_application_install.set_application_id(l_app_id);

  -- Resolve the current Desktop UI ID for the app
  SELECT ui.user_interface_id
    INTO l_ui_id
    FROM apex_application_user_interfaces ui
   WHERE ui.application_id = l_app_id
     AND ui.is_current = 'Yes';
  ...
  wwv_flow_api.import_begin (
    ...
    p_default_owner             => NVL(apex_application_install.get_schema, SYS_CONTEXT('USERENV','CURRENT_SCHEMA'))
  );

  -- Create Teacher Dashboard page 200 (ensure Page 200 is free in App 105)
  wwv_flow_api.create_page(
    p_id                        => 200,
    p_flow_id                   => l_app_id,
    p_user_interface_id         => l_ui_id,
    p_name                      => 'Teacher Dashboard',
    p_step_title                => 'Teacher Dashboard',
    p_autocomplete_on_off       => 'OFF',
    p_last_updated_by           => NVL(apex_application_install.get_schema, SYS_CONTEXT('USERENV','CURRENT_SCHEMA')),
...
prompt Page installed. Open:  f?p=105:200
